var input = document.getElementById('tela'),
  numero = document.querySelectorAll('.varios div'),
...
resultado.addEventListener("click", function() {
  ...
  while (divide != -1) {
    numeros.splice(divide, 2, parseFloat(numeros[divide]) / parseFloat(numeros[divide + 1]));
  ...
  while (multiply != -1) {
    numeros.splice(multiply, 2, parseFloat(numeros[multiply]) * parseFloat(numeros[multiply + 1]));
  ...
  while (subtract != -1) {
    numeros.splice(subtract, 2, parseFloat(numeros[subtract]) - parseFloat(numeros[subtract + 1]));
...
//if é que nem o se
//else é que nem o senão
//else if é que nem senao se